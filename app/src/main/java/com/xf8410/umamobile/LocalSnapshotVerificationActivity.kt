...
class LocalSnapshotVerificationActivity : androidx.activity.ComponentActivity() {
    ...
    private fun verifySelected() {
        ...
        executor.execute {
            val result = runCatching { verify() }
                .fold({ it }, { error -> "本地验收失败\n${error.javaClass.simpleName}: ${error.message.orEmpty()}" })
    ...
    private fun verify(): String {
        ...
        if (!manifestFile.isFile) throw IllegalStateException("缺少 manifest.json：${manifestFile.absolutePath}")
        if (!rawRoot.isDirectory) throw IllegalStateException("缺少 raw/：${rawRoot.absolutePath}")
        ...
        for (index in 0 until expected.length()) {
            ...
            if (actualLength != expectedLength) {
                sizeMismatch += "$relative ($actualLength != $expectedLength)"
                continue
            }
            val actualHash = sha256(file)
            if (indexedHash != null && !actualHash.equals(indexedHash, ignoreCase = true)) {
                hashMismatch += relative
                continue
            }
            verifiedFiles++
            verifiedBytes = Math.addExact(verifiedBytes, actualLength)
            entries.put(JSONObject().apply {
                put("relative_path", relative)
                put("byte_length", actualLength)
        ...
        rawRoot.walkTopDown().filter { it.isFile }.forEach { file ->
            ...
            if (relative.endsWith(".part")) partial += relative
            else if (!seen.contains(relative)) extra += relative
        }
        ...
        val verification = JSONObject().apply {
            ...
            put("verified_file_count", verifiedFiles)
            put("verified_bytes", verifiedBytes)
            put("expected_file_count", expected.length())
            put("missing_files", JSONArray(missing))
            put("size_mismatch_files", JSONArray(sizeMismatch))
            put("sha256_mismatch_files", JSONArray(hashMismatch))
            put("partial_files", JSONArray(partial))
            put("extra_files", JSONArray(extra))
        ...
        val localIndex = JSONObject().apply {
            put("schema_version", 1)
            put("session_id", sessionId)
            put("session_state", state)
            put("status", verificationStatus)
        ...
        atomicWrite(File(root, "local-index.json"), localIndex.toString(2).toByteArray(Charsets.UTF_8))
        return buildString {
            appendLine("本地快照验收完成")
            appendLine("session_id=$sessionId")
            appendLine("status=$verificationStatus")
            appendLine("verified_files=$verifiedFiles/${expected.length()}")
            appendLine("verified_bytes=$verifiedBytes")
            if (missing.isNotEmpty()) appendLine("missing=${missing.joinToString()}")
            if (sizeMismatch.isNotEmpty()) appendLine("size_mismatch=${sizeMismatch.joinToString()}")
            if (hashMismatch.isNotEmpty()) appendLine("sha256_mismatch=${hashMismatch.joinToString()}")
            if (partial.isNotEmpty()) appendLine("partial=${partial.joinToString()}")
            if (extra.isNotEmpty()) appendLine("extra=${extra.joinToString()}")
            append("verification.json 与 local-index.json 已更新")
        }
    }
